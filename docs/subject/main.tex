...
\noindent Exigences de structure (OrderBook) :
\begin{itemize}
  \item Le carnet est organisé en \textbf{niveaux de prix} (price levels).
  \item À chaque niveau de prix, les ordres sont stockés en \textbf{FIFO} (time priority).
  \item Un index interne \textbf{orderId $\rightarrow$ référence} est \textbf{obligatoire} pour implémenter
        \textbf{Cancel} sans parcourir tout le carnet.
  \item Les meilleurs prix (\textit{best bid} / \textit{best ask}) doivent être accessibles \textbf{sans scan complet}
        du carnet.
\end{itemize}
